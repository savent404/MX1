...
static struct {
  ...
  uint8_t start;
  uint8_t end;
} SFIFO = {.fifo = {empty}, .start = 0, .end = 0};
...
int8_t AP_IN(uint16_t *src) {
  ...
  uint32_t i;
  uint16_t *pt;
  if (SFIFO.fifo[pos].fifo_status == full) return -1;
...
uint16_t *AP_OUT(void) {
  ...
  uint16_t *pt = 0;
  // if fifo is empty
  if (SFIFO.fifo[pos].fifo_status == empty) return pt;
...
int8_t AP_Play(void) {
  ...
  HAL_DAC_Start_DMA(&hdac, DAC_CHANNEL_1, (uint32_t *)pt, FIFO_SIZ,
                    DAC_ALIGN_12B_R);
  return 0;
}
 ...
 */
void AP_Triger(void) {
  if (dma_triger && AP_status == running) AP_Play();
  dma_triger = 0;
}
